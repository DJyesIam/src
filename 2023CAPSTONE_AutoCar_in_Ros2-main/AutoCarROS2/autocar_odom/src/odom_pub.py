...
class odomPublisher(Node):
	...
	def __init__(self):
		...
		self.gps_sub = self.create_subscription(NavSatFix, 'gps/fix', self.gps_callback, qos_profile)
		self.gps_vel_sub = self.create_subscription(TwistStamped, 'gps/vel', self.gps_vel_callback, qos_profile)
	...
	def gps_vel_callback(self, gps_vel):
		...
		self.gpose.twist.twist.linear.x = gps_vel.twist.linear.x
		self.gpose.twist.twist.linear.y = gps_vel.twist.linear.y
		self.gpose.twist.twist.linear.z = gps_vel.twist.linear.z

		self.filtered_heading=math.atan2(gps_vel.twist.linear.y , gps_vel.twist.linear.x)
		...
		if (abs(gps_vel.twist.linear.x) or abs(gps_vel.twist.linear.y)) > 0.1:
		...
		self.velocity = np.sqrt(gps_vel.twist.linear.x ** 2 + gps_vel.twist.linear.y ** 2)
		...
		# save gps vel
		self.gps_data.twist.twist.linear.x = gps_vel.twist.linear.x
		self.gps_data.twist.twist.linear.y = gps_vel.twist.linear.y
	...
	def imu_callback(self, imu):
		...
		self.imu_data.angular_velocity.z = imu.angular_velocity.z
